...
if __name__ == "__main__":
    ...
    counts = lines.filter(lambda x: x[8].strip() != "" and x[9].strip() != "") \
    ...
    invalid2 = lines.filter(lambda x: x[8].strip() != "" and x[9].strip() != "") \
                 .map(lambda x: ((x[8], x[9]), 1)) \
                 .reduceByKey(add) \
    ...
    conflicts.coalesce(1).saveAsTextFile("ConflictPDCodeMapping.out")
    invalidOutput.coalesce(1).saveAsTextFile("InvalidPDCodeMapping.out")
